...
for i in range(n_of_matrix - 1):
    ele.append(int(input().split(" ")[0]))
last_input = input().split(" ")
ele.append(int(last_input[0]))
ele.append(int(last_input[1]))
# print(ele)
table = [[0] * n_of_matrix for _ in range(n_of_matrix)]
# print(table)
for i in range(n_of_matrix - 1):
    for y in range(n_of_matrix - i - 1):
        x = i + y + 1
        table[y][x] = min((table[y][y+k] + table[y+k+1][x] + ele[y]*ele[y+k+1]*ele[x+1]) for k in range(i+1))
# print(table)
print(table[0][n_of_matrix-1])
