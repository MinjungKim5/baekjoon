...
int main() {
/////////////////////////////////////////////
int radder, snake; cin >> radder >> snake;
vector<int> adjlist(101,0);
vector<int> visitlist(101, 1000);
for (int i = 0; i < radder+snake; i++) {
    int start, end; cin >> start >> end;
    adjlist[start] = end;
}

queue<int> visitq; visitq.push(1); visitlist[1] = 0;
while(!visitq.empty()) {
    int curr = visitq.front(); visitq.pop();
    int count = visitlist[curr];
    for (int dice = 1; dice <= 6; dice++) {
        if (curr+dice == 100) {cout << count+1; return 0;}
        if (curr+dice > 100) continue;
        if (visitlist[curr+dice] > count+1) {
            visitlist[curr+dice] = count+1;
            if (adjlist[curr+dice]) {
                if(visitlist[adjlist[curr+dice]] > count+1) {
                    visitlist[adjlist[curr+dice]] = count+1;
                    visitq.push(adjlist[curr+dice]);
                }
                continue;
            }
            visitq.push(curr+dice);
        }
    }
}
/////////////////////////////////////////////
