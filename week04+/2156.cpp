...
cin.tie(NULL);
///////////////////////////////
int glasses; cin >> glasses;
vector<int> wine(glasses);
cin >> wine[0];
vector<int> dptable(glasses+2,0);
dptable[2] = wine[0];
for (int i = 1; i < glasses; i++) {
    cin >> wine[i];
    int tempmax=max(dptable[i]+wine[i], dptable[i-1]+wine[i-1]+wine[i]);
    dptable[i+2] = max(tempmax, dptable[i+1]);
}
cout << max(dptable[glasses], dptable[glasses+1]);
///////////////////////////////
return 0;}