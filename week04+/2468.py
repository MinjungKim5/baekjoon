...
gmap = []
for _ in range(size):
    gmap.append(list(map(int,input().strip().split())))
...
def safeZones(level):
    ...
    for i in range(size):
        for j in range(size):
            if gmap[i][j] <= level or visited[i][j]: continue
            q.append([i,j])
            count += 1
            while q:
                y,x = q.popleft()
                for d in dir:
                    newy = y+d[0]
                    newx = x+d[1]
                    if newy < 0 or newy >= size or newx < 0 or newx >= size: continue
                    if gmap[newy][newx] > level and not visited[newy][newx]:
