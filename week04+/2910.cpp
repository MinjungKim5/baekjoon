...
bool compare(pair<int,int> a, pair<int,int> b) {
    ...
    else if (sequence[a.first] < sequence[b.first]) return true;
    else return false;
}
...
int main() {
    ...
    vector<pair<int,int>> numNfreq(cnt.begin(), cnt.end());
    sort(numNfreq.begin(), numNfreq.end(), compare);
    for (pair<int,int> nf: numNfreq) {
        for (int i = 0; i < nf.second; i++) {
            cout << nf.first << " ";
        }
    }
}