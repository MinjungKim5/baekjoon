...
int main() {
//////////////////////
int n_of_groups, m_of_quiz;
cin >> n_of_groups >> m_of_quiz;
unordered_map<string, vector<string>> g2m;
unordered_map<string, string> m2g;

for (int n = 0; n < n_of_groups; n++) {
    string groupname;
    cin >> groupname;
    int members;
    cin >> members;
    string name;
    vector<string> assemble(members);
    for (int m = 0; m < members; m++) {
        cin >> name;
        m2g.insert({name, groupname});
        assemble[m] = name;
    }
    sort(assemble.begin(), assemble.end());
    g2m.insert({groupname, assemble});
}

for (int q = 0; q < m_of_quiz; q++) {
    string question;
    bool ask4group;
    cin >> question >> ask4group;
    if (ask4group) cout << m2g[question] << endl;
    else {
        vector<string> answer = g2m[question];
        for (int m = 0; m < answer.size(); m++) cout << answer[m] << endl;
    }
}
