...
int safeZones(int level) {
    ...
    for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
            if (map[i][j] <= level || visited[i][j]) continue;
            q.push({i,j});
            count++;
            while (!q.empty()) {
                int y = q.front().first;
                ...
                for (int d = 0; d < 4; d++) {
                    ...
                    if (map[newy][newx] > level && !visited[newy][newx]) {
...
for (int i = 1; i <= 100; i++) {
    if (!nums[i]) continue;
    int safe = safeZones(i);
    if (maxval < safe) maxval = safe;
}
cout << maxval;
///////////////////////////////////////////
return 0;}