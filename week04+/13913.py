...
def checkNmark(target, idx):
    if visited[target][1] == 0:
        ...
        visited[target][1] = idx
        willvisit.append(target)

if iszero: visited[k][0] = 1; n = 1
else: visited[k][0] = 0 
visited[k][1] = 10000000
...
while len(willvisit) > 0:
    ...
    if visited[n][1] !=0:
        print(visited[n][0])
        if iszero: print(0, end= " ")
        print(n, end=" ")
        origin = visited[n][1]
        while origin != 10000000:
            print(origin, end= " ")
            origin = visited[origin][1]
        exit()