...
for test in range(test_case):
    ...
    # dptable[i][j]는 i번 파일부터 j 번 파일까지 합쳤을 때 최소비용
    dptable = [([0]*nOfFiles) for _ in range(nOfFiles)]
    sumtable = [0]*(nOfFiles+1)
    sum = 0
    for i in range(nOfFiles):
        sum += files[i]
        sumtable[i] = sum
    for diff in range(1, nOfFiles):
        for i in range(nOfFiles-diff):
            minval = 2000000000
            for j in range(diff):
                res = dptable[i][i+j]+dptable[i+j+1][i+diff]
                if res < minval: minval = res
            dptable[i][i+diff] = minval + sumtable[i+diff] - sumtable[i-1]
    print(dptable[0][nOfFiles-1])