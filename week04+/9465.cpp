...
for (int t = 0; t < testcase; t++) {
    ...
    if (n == 1) {cout << max<int>(nums[0][0], nums[1][0]) << "\n"; continue;}
    vector<vector<int>> dp(2, vector<int>(n, 0));
    dp[0][0] = nums[0][0];
    dp[1][0] = nums[1][0];
    dp[0][1] = dp[1][0] + nums[0][1];
    dp[1][1] = dp[0][0] + nums[1][1];
    int maxval = max<int>(dp[0][1], dp[1][1]);
    for (int i = 2; i < n; i++) {
        int leftleftmax = max<int>(dp[0][i-2],dp[1][i-2]);
        dp[0][i] = max<int>(leftleftmax,dp[1][i-1])+nums[0][i];
        if (maxval < dp[0][i]) maxval = dp[0][i];
        dp[1][i] = max<int>(leftleftmax,dp[0][i-1])+nums[1][i];
