...
n = int(input())
sols = list(map(int, input().strip().split()))
left, right = 0, n-1
res = [abs(sols[0] + sols[n-1]), 0, n-1]
while(right > left+1):
    leftMove = abs(sols[left+1]+sols[right])
    rightMove = abs(sols[left]+sols[right-1])
    if (leftMove < rightMove):
        if (leftMove < res[0]): res = [leftMove, left+1, right]
        left += 1
    else:
        if (rightMove < res[0]): res = [rightMove, left, right-1]
        right -=1
print(f"{sols[res[1]]} {sols[res[2]]}")