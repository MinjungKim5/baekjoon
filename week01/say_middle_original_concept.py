...
mid_num = int(sys.stdin.readline())
list = [mid_num]
print(mid_num)
mid_index = 0
...
for i in range(first_input - 1):
    ...
    insert_index = bisect.bisect(list, input_num)

    list.insert(insert_index, input_num)
    #짝수에서 넣을때
    if i % 2 == 1:
        mid_index += 1
        if input_num <= mid_num: print(mid_num)
        else:
            mid_num = list[mid_index]
            print(mid_num)

    #홀수에서 넣을때
    elif mid_num <= input_num: print(mid_num)
    else:
        mid_num = list[mid_index]
        print(mid_num)