...
def quad(bitmap, length):
    # print(bitmap)
    isunison = True
    if length == 1: print(bitmap[0], end=""); return
    for i in range(length-1):
        if bitmap[0][i] != bitmap[0][i+1] or bitmap[i] != bitmap[i+1]:
            isunison = False
            break
    if isunison:
        print(bitmap[0][0], end="")
        return
    bitmap1 = []
    bitmap2 = []
    bitmap3 = []
    ...
    for j in range(length//2):
        bitmap1.append(bitmap[j][0:length//2])
        bitmap2.append(bitmap[j][length//2:])
        bitmap3.append(bitmap[j+length//2][0:length//2])
        bitmap4.append(bitmap[j+length//2][length//2:])
    print("(",end="")
    quad(bitmap1, length//2)
    quad(bitmap2, length//2)
    quad(bitmap3, length//2)
    quad(bitmap4, length//2)
    print(")",end="")

n_of_lines = int(input())
bitmap = []
for _ in range(n_of_lines):
    bitmap.append(input().strip())
quad(bitmap, n_of_lines)