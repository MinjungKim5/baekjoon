...
for _ in range(first_input):
    ...
    n_of_nodes = int(case_input[0])
    m_of_edges = int(case_input[1])

    adj_list = [[] for _ in range(n_of_nodes)]
    for i in range(m_of_edges):
        edge = sys.stdin.readline().split(" ")
        adj_list[int(edge[0])-1].append(int(edge[1])-1)
        adj_list[int(edge[1])-1].append(int(edge[0])-1)
    checked_list = [0 for _ in range(n_of_nodes)]
    def check_triangle(node):
        for adj_node in adj_list[node]:
            if checked_list[adj_node] == 1: continue
            for adj2_node in adj_list[adj_node]:
                if adj2_node == node: continue
                if checked_list[adj2_node] == 1: continue
                if node in adj_list[adj2_node]: return False
                else:
                    checked_list[node] = 1
        return True
                
    res = True
    for node in range(n_of_nodes):
        if check_triangle(node) == False:
            res = False
            break
